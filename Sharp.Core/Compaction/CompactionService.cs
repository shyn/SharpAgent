...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (firstKeptEntryId != null)
        {
            var skippedEntries = entries as List<SessionEntryEnvelope> ?? entries.ToList();
            var conversation = BuildConversation(skippedEntries.GetRange(compactedEntryIds.Count, skippedEntries.Count - compactedEntryIds.Count));
