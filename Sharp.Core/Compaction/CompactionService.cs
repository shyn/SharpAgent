...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        for (int i = 0; i < entryCutPoint; i++) compactedEntries.Add(entries[i]);

        var keptEntries = new List<SessionEntryEnvelope>(entries.Count - entryCutPoint);
        for (int i = entryCutPoint; i < entries.Count; i++) keptEntries.Add(entries[i]);
        ...
        CancellationToken ct = default)
    {
        ...
        if (firstKeptEntryId != null)
        {
            var conversation = BuildConversation(entries, compactedEntryIds.Count);
    ...
    /// </summary>
    private static List<LlmMessage> BuildConversation(IReadOnlyList<SessionEntryEnvelope> entries, int startIndex = 0)
        => BuildConversationEntries(entries, startIndex).ConvertAll(e => e.Message);

    private static List<ConversationEntry> BuildConversationEntries(IReadOnlyList<SessionEntryEnvelope> entries, int startIndex = 0)
    {
