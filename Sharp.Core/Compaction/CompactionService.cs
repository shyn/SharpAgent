...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Optimization: Use ConvertAll instead of Select().ToList() to avoid enumerator overhead and intermediate allocations
        ...
        // Optimization: Pre-allocate List capacity and use a standard loop to prevent intermediate array allocations
        ...
        // Optimization: Use GetRange instead of Take().ToList() for direct array slice extraction
        ...
        CancellationToken ct = default)
    {
        ...
        if (firstKeptEntryId != null)
        {
            ...
            // Optimization: Pre-allocate List capacity and use a standard loop instead of Skip().ToList()
            var keptEntriesForConversation = new List<SessionEntryEnvelope>(entries.Count - skipCount);
            for (var i = skipCount; i < entries.Count; i++)
            {
                keptEntriesForConversation.Add(entries[i]);
            }
            var conversation = BuildConversation(keptEntriesForConversation);
    ...
    private static List<LlmMessage> BuildConversation(IReadOnlyList<SessionEntryEnvelope> entries)
        => BuildConversationEntries(entries).ConvertAll(e => e.Message); // Optimization: ConvertAll over Select().ToList()
