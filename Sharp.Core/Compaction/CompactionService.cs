...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (firstKeptEntryId != null)
        {
            ...
            var keptForHook = new List<SessionEntryEnvelope>(entries.Count - skipCount);
            for (int i = skipCount; i < entries.Count; i++)
            {
                keptForHook.Add(entries[i]);
            }
            var conversation = BuildConversation(keptForHook);
