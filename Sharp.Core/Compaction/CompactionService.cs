...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (firstKeptEntryId != null)
        {
            var uncompactedEntries = new List<SessionEntryEnvelope>(entries.Count - compactedEntryIds.Count);
            for (var i = compactedEntryIds.Count; i < entries.Count; i++)
                uncompactedEntries.Add(entries[i]);

            var conversation = BuildConversation(uncompactedEntries);
