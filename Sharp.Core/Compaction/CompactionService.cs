...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (firstKeptEntryId != null)
        {
            var skippedCount = compactedEntryIds.Count;
            var keptEntries = new List<SessionEntryEnvelope>(entries.Count - skippedCount);
            for (var i = skippedCount; i < entries.Count; i++)
            {
                keptEntries.Add(entries[i]);
            }
