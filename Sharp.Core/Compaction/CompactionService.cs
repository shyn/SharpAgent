...
public sealed class CompactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var compactedConversation = new List<LlmMessage>(cutPoint);
        for (var i = 0; i < cutPoint; i++)
        {
            compactedConversation.Add(conversation[i]);
        }

        var summary = await GenerateSummaryAsync(compactedEntries, compactedConversation, model, systemPrompt, ct);
