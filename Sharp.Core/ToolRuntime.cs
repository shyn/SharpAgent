...
public sealed class ToolRuntime
{
    ...
    {
        var capacity = tools is IReadOnlyCollection<IAgentTool> collection ? collection.Count : 0;
        _toolsByName = new Dictionary<string, IAgentTool>(capacity, StringComparer.Ordinal);

        foreach (var tool in tools)
            _toolsByName[tool.Name] = tool;

        var toolDefs = new List<ToolDefinition>(_toolsByName.Count);
        foreach (var tool in _toolsByName.Values)
        {
            toolDefs.Add(new ToolDefinition(tool.Name, tool.Description, tool.ParametersSchema));
        }
        _toolDefinitions = toolDefs;
