...
public sealed class OpenAiResponsesLlmProvider : ILlmProvider
{
    ...
    private static OpenAiResponsesRequest BuildRequestPayload(LlmRequest request)
    {
        ...
        List<OpenAiResponsesTool>? openAiTools = null;
        if (request.Tools.Count > 0)
        {
            openAiTools = new List<OpenAiResponsesTool>(request.Tools.Count);
            foreach (var tool in request.Tools)
            {
                openAiTools.Add(ToOpenAiTool(tool));
            }
        }
                ...
                case LlmMessageRole.Assistant:
                    {
                        ...
                        var toolCalls = new List<ToolCallContentBlock>();
