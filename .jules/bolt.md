...
## 2024-05-15 - [Cache stable generated collections instead of repeated LINQ evaluations]
**Learning:** Returning LINQ evaluations (like `.Select().ToList()`) from frequently called properties or parameterless methods (like `ToolRuntime.ToToolDefinitions()`) introduces hidden O(N) object and array allocations that execute on every call. In high-frequency hot paths like the `AgentLoop`, this generates significant garbage and degrades performance.
**Action:** In C# performance-critical paths, always cache stable collections derived from internal state during construction (e.g., storing an `IReadOnlyList<T>`) rather than re-evaluating LINQ queries.
