...
## 2024-11-20 - [Zero-Allocation in C# Hot Paths via Pre-Sized Lists and Array Copies]
**Learning:** In C# hot paths, using LINQ operations like `.Select().ToList()`, `.Take().ToList()`, or `.Skip().ToList()` causes massive GC pressure due to hidden enumerator allocations and intermediate array copies.
**Action:** When extracting data in performance-critical code (such as AgentLoop, CompactionService, or ToolRuntime), manually instantiate a `List<T>` with a known capacity and use a `foreach` or `for` loop to append elements, or use `List<T>.GetRange()` to directly slice underlying arrays without executing enumerators.
