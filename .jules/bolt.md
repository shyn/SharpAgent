...
## 2025-03-04 - [Performance] Eliminating hidden allocations in C# hot paths
**Learning:** LINQ methods like `.Select().ToList()`, `.Take().ToList()`, and `.Skip().ToList()` introduce hidden overhead (enumerator allocation, intermediate list creation, array copies) on hot paths such as `AgentLoop` and `CompactionService`.
**Action:** Always prefer native `List<T>` operations like `.ConvertAll()`, `.GetRange()`, or the `List<T>` copy constructor (`new List<T>(collection)`) when transforming or taking sub-ranges of collections in high-frequency contexts to reduce GC pressure.
