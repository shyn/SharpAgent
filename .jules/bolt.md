...
## 2025-03-04 - Removing LINQ ToList() allocations using List<T> methods
**Learning:** Found several hot spots in `CompactionService.cs` where multiple `LINQ` operations like `.Select().ToList()`, `.Take().ToList()`, and `.Skip().ToList()` chained together, causing unnecessary memory allocations.
**Action:** Replaced these chains with highly optimized underlying `List<T>` methods. Using `.ConvertAll()` immediately projects items with the exact capacity needed without an enumerator, while `GetRange()` leverages fast array copying instead of allocating intermediate objects. Always type-check `IReadOnlyList<T>` to leverage `List<T>` extensions or fallback to loop iterations for performance.
