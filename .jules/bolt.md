...
## 2025-06-07 - Caching lists derived from dictionaries avoids redundant memory allocations
**Learning:** Returning `IReadOnlyList<T>` from a method that computes the collection using `.Select().ToList()` on dictionary values (e.g., `ToolRuntime.ToToolDefinitions()`) introduces a hidden, recurring memory allocation bottleneck in hot paths.
**Action:** When a stable collection is derived from internal state (like a dictionary), compute the collection once during class construction and cache it as a private readonly field. Ensure the list is populated *after* the source dictionary has been fully built to preserve any implicit deduplication or overwrite semantics, and simply return the cached list in subsequent method calls.
