...
## 2026-06-24 - [Cache ToolDefinition on ToolRuntime instantiation]
**Learning:** Returning LINQ queries like `.Select().ToList()` from simple getters or properties on hot paths (e.g., `ToToolDefinitions()`) forces hidden recurring list allocations and garbage collection overhead. Since `ToolRuntime` is constructed once and tools do not change dynamically, we can cache this list during construction.
**Action:** When collections are stable after initialization, evaluate caching them as fields using an explicit loop with a predefined capacity during the constructor phase. This avoids re-evaluation and reallocation every time the property or getter is accessed.
