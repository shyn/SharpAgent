...
## 2025-05-18 - [Avoid micro-optimizing pre-network cold paths]
**Learning:** I attempted to optimize LINQ allocations (`.Select().ToList()`) inside `CompactionService.cs`. However, these allocations were immediately followed by a network request to an LLM (`GenerateSummaryAsync`). The microsecond GC pressure saved was entirely overshadowed by the multi-second latency of the network call. This violates the rule to avoid micro-optimizing cold paths.
**Action:** When evaluating LINQ replacements, check what happens immediately after the code block. If it awaits an I/O bound network request (like LLM payload generation), it is a cold path and not worth optimizing at the cost of readability.
