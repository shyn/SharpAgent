...
## 2025-03-04 - Cache stable collections during construction
**Learning:** Found that `ToolRuntime.ToToolDefinitions()` was generating a new list of `ToolDefinition` objects via LINQ `.Select().ToList()` on every call. In hot paths like `AgentLoop` where the agent repeatedly fetches tools, this caused hidden recurring allocations. Since tool definitions are stable per runtime instance, this is wasteful.
**Action:** In C# performance-critical components, cache stable collections generated from internal state during construction (e.g., storing an `IReadOnlyList<ToolDefinition>`) rather than re-evaluating LINQ expressions in properties or parameterless methods to eliminate hidden recurring list allocations.
