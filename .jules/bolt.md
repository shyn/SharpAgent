...
## 2025-03-05 - Avoid hidden LINQ allocations on hot paths by caching immutable derived collections during construction
**Learning:** Found an anti-pattern in `ToolRuntime` where a parameterless method `ToToolDefinitions()` was invoking `.Select(...).ToList()` on every call. Because this method was called from the high-frequency `AgentLoop` hot path, it caused continuous redundant O(N) list allocations and LINQ closures for static data.
**Action:** When a collection derived from internal state (like a dictionary of tools) is immutable or stable, calculate and cache the resulting list (e.g., as `IReadOnlyList<T>`) during construction or state changes, rather than recalculating it on demand via LINQ in properties or parameterless methods. Ensure the build order preserves implicit business logic (like deduplication).
