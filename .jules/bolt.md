...
## 2025-03-05 - Avoid redundant ToList allocations in mapping properties
**Learning:** Returning LINQ expressions like `.Select(...).ToList()` from properties or parameterless methods (e.g. `ToToolDefinitions()`) generates hidden garbage per invocation.
**Action:** When a method returns a stable collection generated from internal state, evaluate caching the generated list internally during construction (e.g., storing `_toolDefinitions`) and simply returning the pre-built `IReadOnlyList<T>` to eliminate recurring list allocations on hot paths.
