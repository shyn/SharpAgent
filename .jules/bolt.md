...
## 2025-03-04 - Removing .ToList() allocations in CompactionService by replacing LINQ calls with ConvertAll and GetRange
**Learning:** Found multiple usages of LINQ operators like `.Select(e => e.Message).ToList()`, `.Take().ToList()`, and `.Skip().ToList()` in `CompactionService` in `Sharp.Core`. These cause unnecessary intermediate array allocations, memory garbage and enumerator overhead for collections where length and capacity are known in advance.
**Action:** Use native collection methods like `ConvertAll()`, `GetRange()`, or a standard `for` loop with a pre-sized list to eliminate intermediate garbage allocations in collection hot paths.
