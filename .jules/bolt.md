...
## 2025-03-04 - [Cache collections derived from dictionaries during construction]
**Learning:** Found an anti-pattern where a `.Select().ToList()` LINQ allocation was evaluated dynamically in a parameterless method `ToToolDefinitions()` on every call in the high-frequency `AgentLoop` hot path. This created hidden, recurring object and list allocations that could easily be cached.
**Action:** When a static collection is derived from a dictionary and accessed on a hot path, build and cache the collection as a readonly field during construction, *after* the dictionary is fully populated, to preserve any implicit deduplication logic (like overwriting duplicate keys) while eliminating hot-path allocations.
